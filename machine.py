...
TOTAL_NUM_MACHINES = 2 # total number of model machines
...
def socket_thread(sock, message_queue, log_file):
    while True:
        ...
        if message:
            message_queue.append(message.decode(encoding=ENCODING))
            print('Message_Queue: {}'.format(message_queue))
            log_file.write('New Message: {}\n'.format(message.decode(encoding=ENCODING)))
        else:
        ...
        for ins_num in range(clock_rate):
            ...
            if message_queue:
                ...
                if dice == 1:
                    ...
                    try:
                        client_sockets[1].send(str(logical_clock).encode(encoding=ENCODING))
                    except:
                        continue
                    logical_clock += 1
